      ...
      {settings.tajwid && (
        <div className='flex flex-col items-center w-full mt-4'>
          <button
            ...
            className='flex items-center justify-center gap-1.5 text-[11px] md:text-xs text-[#1e3a8a] dark:text-blue-400 font-bold hover:underline mb-3'
          >
          ...
          {showTajwidInfo && (
            <div className='w-full max-w-md mx-auto md:max-w-none md:mx-0 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 pb-2'>
              {TAJWID_RULES.map((r) => (
                <div key={r.key} className='flex items-start gap-3 p-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm'>
                  <span
                  ...
                  <div className='flex-1 min-w-0'>
                    ...
                    <p className='text-[10px] md:text-xs text-slate-400 dark:text-slate-500 leading-relaxed'>
                      {r.desc}
                    </p>
                  </div>
                </div>
