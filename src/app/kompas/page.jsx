...
export default function KompasPage() {
  ...
  useEffect(() => {
    const loadSavedCity = async () => {
      ...
      const profileCity = profile?.location || '';
      setSavedCity(profileCity);
      if (profileCity) {
        setCity(profileCity);
      }
    };
      ...
      (pos) => {
        ...
        fetch(`/api/reverse-geocode?lat=${latitude}&lon=${longitude}`)
          ...
          .then((d) => {
            ...
            setCity(savedCity || 'Lokasi GPS aktif');
          })
          .catch(() => setCity(savedCity || 'Lokasi GPS aktif'));
      },
      () => {
        setLocating(false);
