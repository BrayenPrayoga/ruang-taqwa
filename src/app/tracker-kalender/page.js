...
export default function TrackerKalender() {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState({ isOpen: false, habitId: null, habitName: '' });
  ...
  const handleDeleteCustomHabit = async (id) => {
    ...
    setDeleteConfirm({ 
      isOpen: true, 
      habitId: id, 
      habitName: habit?.label || 'target tambahan' 
    });
  };

  const confirmDeleteHabit = async () => {
    const { habitId } = deleteConfirm;
    const updatedHabits = customHabits.filter((h) => h.id !== habitId);
    setCustomHabits(updatedHabits);
    await localforage.setItem('custom_habits', updatedHabits);
    setDeleteConfirm({ isOpen: false, habitId: null, habitName: '' });
  };
  ...
  return (
    <main className='min-h-screen bg-[#F6F9FC] dark:bg-slate-950 pb-16 transition-colors duration-300'>
      ...
      <ConfirmDialog
        ...
        onClose={() => setDeleteConfirm({ isOpen: false, habitId: null, habitName: '' })}
        onConfirm={confirmDeleteHabit}
        title="Hapus Target Tambahan"
        message={`Apakah Anda yakin ingin menghapus target "${deleteConfirm.habitName}"? Tindakan ini tidak dapat dibatalkan.`}
        confirmText="Hapus"
        cancelText="Batal"
      />
    </main>
